...
def compute_bayes_risk(n_test):
    X_test = np.random.uniform(xmin, xmax, n_test).reshape(n_test, 1)
    modulation = (1-X_test/xmax)
    noise = rng.normal(0, std, (n_test, 1))
    y_pred = modulation*(np.sin(8*X_test))
    y_truth = modulation*np.sin(8*X_test)+noise
    diff = y_pred-y_truth
    test_error = 1/n_test*np.linalg.norm(diff)**2
    return test_error
...
n_test = 10**6
# noise = rng.normal(0, std, (n_test, 1))
# norm = 1/len(noise)*np.linalg.norm(noise)**2
# print(f"noise norm: {norm}")
...
figtitle = "Random_forest_regression"
figtitle+=params_fig
